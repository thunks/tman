...
function Tman () {
  ...
  this.rootSuite.stat = [0, 0] // passed, ignored
  this.rootSuite.errors = [] // errors
...
function tman () {
  ...
  ctx.run = function (callback) {
    ...
    running = true
    rootSuite.stat = [0, 0] // passed, ignored
    rootSuite.errors = [] // errors
    rootSuite.abort = false
    return thunk(rootSuite)(function (err) {
      if (err) throw err

      var res = rootSuite.stat
      var errors = rootSuite.errors
      var time = rootSuite.endTime - rootSuite.startTime
      var message = format.reset('\nTest ' + (errors.length ? 'failed: ' : 'finished: '))
      message += format[res[0] ? 'green' : 'gray'](res[0] + ' passed; ', true)
      message += format[errors.length ? 'red' : 'gray'](errors.length + ' failed; ', true)
      message += format[res[1] ? 'cyan' : 'gray'](res[1] + ' ignored.', true)
      console.log(message, format.yellow('(' + time + 'ms)', true), format.reset('\n'))

      errors.forEach(function (err) {
        console.log(format.indent(1) + format.red(err.order + ') ' + err.title + ':', true))
        var message = err.stack ? err.stack : String(err)
        console.log(message.replace(/^/gm, format.indent(2)) + '\n')
      })
      if (errors.length) console.log(format.reset('\n'))

      return {stat: res, time: time, errors: errors}
    })(callback || finished)
  }
