...
Diff.prototype.logError = function (rootSuite) {
  rootSuite.errors.forEach((err, i) => {
    ...
    let isShowDiff = err.showDiff !== false && sameType(actual, expected) && expected !== undefined
    if (isShowDiff) {
    ...
    let result = errMessageFormat(isShowDiff, (i + 1), err.title, msg, stack)
    this.log(result)
  })
}
...
function stringify (obj) {
  ...
  return typeof obj === 'string' ? obj : JSON.stringify(obj, Object.keys(obj).sort(), 2).replace(/,(\n|$)/g, '$1')
}
